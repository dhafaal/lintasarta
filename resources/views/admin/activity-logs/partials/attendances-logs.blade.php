<div id="attendances-logs" class="tab-content">
    <div class="mb-4">
        <h3 class="mb-2 text-lg font-semibold text-gray-900">
            <i data-lucide="user-check" class="mr-2 inline h-5 w-5 text-sky-600"></i>
            Attendances Logs
        </h3>
        ...
        <div class="space-y-4">
            @foreach ($logs as $log)
                <div class="rounded-lg border-l-4 border-sky-500 bg-gray-50 p-4">
                    <div class="flex items-start justify-between">
                        <div class="flex-1">
                            <div class="mb-2 flex items-center space-x-2">
                                <span
                                    class="{{ $log->action == 'checkin' ? 'bg-green-100 text-green-800' : '' }} {{ $log->action == 'checkout' ? 'bg-blue-100 text-blue-800' : '' }} {{ $log->action == 'absent' ? 'bg-red-100 text-red-800' : '' }} rounded-full px-2 py-1 text-xs font-medium"
                                >
                            ...
                            <div class="grid grid-cols-1 gap-3 text-xs text-gray-600 md:grid-cols-2">
                                @if (isset($add['schedule_date']))
                                    <div>
                                        <i data-lucide="calendar" class="mr-1 inline h-3 w-3"></i>
                                        Tanggal:
                                        {{ \Illuminate\Support\Carbon::parse($add['schedule_date'])->format('d M Y') }}
                                    </div>
                                @endif
                                ...
                                @if (isset($add['distance_m']))
                                    <div>
                                        <i data-lucide="navigation" class="mr-1 inline h-3 w-3"></i>
                        ...
                        <div class="ml-4">
                            <button
                                onclick="toggleDetails('attendance-{{ $log->id }}')"
                    ...
                    <div id="attendance-{{ $log->id }}" class="mt-4 hidden border-t border-gray-200 pt-4">
...
{{ json_encode($log->additional_data, JSON_PRETTY_PRINT) }}</pre
    ...
    @else
        <div class="py-12 text-center">
            ...
            <h3 class="mb-2 text-lg font-medium text-gray-900">No Attendance Logs Found</h3>
