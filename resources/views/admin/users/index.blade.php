...
<x-section-content title="Users" subtitle="Manage all users data and Export everywhere">
    <x-slot:actions>
        <form method="GET" action="{{ route('admin.users.index') }}" id="filter-form" class="flex flex-wrap gap-4 items-center">
            {{-- Role Dropdown --}}
            <x-custom-dropdown 
                name="role" 
                :options="['All' => 'All', 'Admin' => 'Admin', 'Operator' => 'Operator', 'User' => 'User']" 
            ...
            <div>
                <select name="shift" id="shift" class="border border-gray-300 rounded-lg px-3 py-2 text-sm">
                    @foreach (['All', 'Morning', 'Afternoon', 'Night'] as $shift)
            ...
            {{-- Search --}}
            <div>
                <input type="text" name="search" id="search" placeholder="Cari user..." value="{{ request('search') }}"
                    class="border border-gray-300 rounded-lg px-3 py-2 text-sm" />
            </div>

            {{-- Buttons --}}
            <div class="flex gap-2">
                <a href="{{ route('admin.users.exportPdf') }}" class="btn btn-outline">Export</a>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function handleFilterChange(e) {
            ...
            .catch(err => {
                usersTableContainer.innerHTML = '<div class="text-center text-red-600 py-10">Failed to load data.</div>';
