...
<div id="attendance-detail-modal" class="fixed inset-0 z-50 hidden bg-black/50 p-4" style="display: none">
    <div class="mx-auto my-8 max-h-[90vh] w-full max-w-5xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
        <div class="sticky top-0 z-10 rounded-t-2xl border-b-2 border-gray-100 bg-white p-6">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-3">
                    <div id="modal-icon" class="rounded-xl p-3"></div>
                    <div>
            ...
            <div id="filter-section" class="mt-4 space-y-3" style="display: none">
                ...
                <div class="relative">
                    ...
                    <input
                        ...
                        class="w-full rounded-lg border border-gray-300 py-2 pr-4 pl-10 text-sm focus:border-sky-500 focus:ring-2 focus:ring-sky-500"
                    />
                </div>
                ...
                <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                    ...
                    <div class="relative">
                        ...
                        <select
                            id="filter-category"
                            class="w-full appearance-none rounded-lg border border-gray-300 bg-white py-2 pr-4 pl-10 text-sm focus:border-sky-500 focus:ring-2 focus:ring-sky-500"
                        >
                    ...
                    <div class="relative">
                        ...
                        <input
                            type="text"
                            id="filter-shift-search"
                            placeholder="Cari atau pilih nama shift..."
                            class="w-full rounded-lg border border-gray-300 py-2 pr-10 pl-10 text-sm focus:border-sky-500 focus:ring-2 focus:ring-sky-500"
                            autocomplete="off"
                        />
                        <i
                            data-lucide="chevron-down"
                        ...
                        <div
                            id="shift-dropdown"
                            class="absolute top-full right-0 left-0 z-20 mt-1 hidden max-h-48 overflow-y-auto rounded-lg border border-gray-300 bg-white shadow-lg"
                        >
                ...
                <div class="flex justify-end">
                    <button
                        type="button"
                        id="reset-filters"
                        class="flex items-center space-x-2 rounded-lg bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-200"
                    >
        ...
        <div id="modal-content" class="p-6">
            <div class="flex items-center justify-center py-12">
                <div class="h-12 w-12 animate-spin rounded-full border-b-2 border-sky-600"></div>
            </div>
        </div>
    </div>
</div>
    ...
    function renderAttendanceDetail(response) {
        ...
        `;
            ...
            shift.employees.forEach((employee) => {
                ...
                const sum = employeeShiftSummary.get(employee.name) || {
                    count: 1,
                    isDouble: false,
                    isLong: false,
                    shifts: [],
                ...
                if (isServerDouble) {
                    ...
                    const earliestEntry = serverShifts
                        .slice()
                        .sort((a, b) => (a.shift_start || '23:59').localeCompare(b.shift_start || '23:59'))[0];
                    if (earliestEntry && earliestEntry.status) {
                        // Use normalized status from server (already normalized in controller)
                        const lbl =
                            earliestEntry.status === 'izin' && earliestEntry.permission_type
                    ...
                    const earliestEntry = sum.shifts
                        .slice()
                        .sort((a, b) => (a.shift_start || '23:59').localeCompare(b.shift_start || '23:59'))[0];
                    if (earliestEntry && earliestEntry.status) {
                ...
                if (isServerDouble) {
                    const ins = serverShifts
                        .map((s) => s.check_in)
                        .filter(Boolean)
                        .sort();
                    const outs = serverShifts
                        .map((s) => s.check_out)
                        .filter(Boolean)
                        .sort();
                    ...
                    const validIns = times
                        .map((t) => t.ci)
                        .filter(Boolean)
                        .sort();
                    const validOuts = times
                        .map((t) => t.co)
                        .filter(Boolean)
                        .sort();
                ...
                let shiftChips = '';
                if (isServerDouble) {
                    const ordered = serverShifts
                        .slice()
                        .sort((a, b) => (a.shift_start || '23:59').localeCompare(b.shift_start || '23:59'));
                    const chips = ordered.map((s) => `${s.category} • ${s.shift_name}`);
                    shiftChips = chips
                        .map(
                            (ch) =>
                                `<span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium bg-sky-50 text-sky-700 border border-sky-200">${ch}</span>`
                        )
                ...
                if (isServerDouble) {
                    ...
                    if (names.length >= 2) shiftPair = `${names[0]} - ${names[1]}`;
                    else shiftPair = names.join('');
                } else if (sum.isDouble && sum.shifts.length > 1) {
                    const ordered = sum.shifts
                        .slice()
                        .sort((a, b) => (a.shift_start || '23:59').localeCompare(b.shift_start || '23:59'));
                    const names = ordered.map((s) => s.shift_name).filter(Boolean);
                    if (names.length >= 2) shiftPair = `${names[0]} - ${names[1]}`;
