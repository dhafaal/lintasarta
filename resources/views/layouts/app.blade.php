...
<body class="bg-sky-50 min-h-screen font-sans antialiased">
            ...
            <div :class="sidebarCollapsed ? 'p-2' : 'p-6'">
                ...
                <nav class="space-y-1">
                    ...
                    <div class="space-y-1">
                        ...
                        <button
                            @click="sidebarCollapsed ? window.location.href = '{{ route('admin.users.index') }}' : usersExpanded = !usersExpanded"
                            :class="sidebarCollapsed ? 'justify-center px-2 py-3' : 'px-3 py-2'"
                    ...
                    <div class="space-y-1">
                        <!-- Modified click behavior to navigate when collapsed, expand when not collapsed -->
                        <button
                            @click="sidebarCollapsed ? window.location.href = '{{ route('admin.schedules.index') }}' : schedulesExpanded = !schedulesExpanded"
                            :class="sidebarCollapsed ? 'justify-center px-2 py-3' : 'px-3 py-2'"
                            class="group flex items-center w-full text-sm font-medium rounded-md transition-all duration-150 relative
                    ...
                    <div class="space-y-1">
                        <!-- Modified click behavior to navigate when collapsed, expand when not collapsed -->
                        <button
                            @click="sidebarCollapsed ? window.location.href = '{{ route('admin.shifts.index') }}' : shiftsExpanded = !shiftsExpanded"
                            :class="sidebarCollapsed ? 'justify-center px-2 py-3' : 'px-3 py-2'"
                            class="group flex items-center w-full text-sm font-medium rounded-md transition-all duration-150 relative
   ...
   {{ request()->routeIs('admin.calendar.view') ? 'bg-sky-500 text-white border border-sky-400' : 'text-sky-100 hover:bg-sky-500 hover:text-white border border-transparent hover:border-sky-400' }}"
                        ...
                        <svg class="w-5 h-5 {{ request()->routeIs('admin.calendar.view') ? 'text-white' : 'text-sky-200 group-hover:text-white' }}"
                            :class="sidebarCollapsed ? 'mr-0' : 'mr-3'" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
